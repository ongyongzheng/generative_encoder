...
from models.utils.image_transform import NumpyResize, NumpyToTensor, pil_loader, standardTransform
...
def test(parser):
    ...
    while best > threshold:
        ...
        if with_ae:
            ...
            encoded = 0.01 * torch.nn.MSELoss()(encoded, encoded_real)
            loss = reconstruction + encoded
        else:
            reconstruction = torch.nn.MSELoss()(output, image_real)
            loss = reconstruction
        ...
        if (i+1) % 100 == 0:
            ...
            if best is None:
                ...
                transform(torch.clamp(output.detach().cpu(), min=-1, max=1)[0]).save("./results/{}/bulk_test_{}.jpg".format(evaluation_id, key))
                best_vector = sv.vector.cpu().detach().numpy()
