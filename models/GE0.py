...
class GE0(object):
    ...
    def build_model(self):
        ...
        #self.discriminator.decoder = None # we only take the encoder portion
        # load weights
        self.generator.load_state_dict(torch.load(self.weight_dir + 'gen'))
        self.discriminator.encoder.load_state_dict(torch.load(self.weight_dir + 'ae_enc'))
        self.discriminator.decoder.load_state_dict(torch.load(self.weight_dir + 'ae_dec'))

    def test(self, test_data): # in GE0, we do not have a train function. instead, we only have test
        # set to eval
        ...
        for step, data in enumerate(test_data):
            ...
            for i in range(num_iterations):
                z_optim.zero_grad()
                m = self.discriminator.encoder(data.to(self.device))
                ge = self.discriminator.encoder(self.generator(z))

                loss = nn.MSELoss()(ge, m) + 0.1*torch.norm(z)
                loss.backward()
                z_optim.step()
                if (i+1) % 100 == 0:
                    ...
                    if step == (num_to_test - 1):
                        break
                    fig.savefig(self.weight_dir + 'ge0_fig.png', format='png', dpi=300)
